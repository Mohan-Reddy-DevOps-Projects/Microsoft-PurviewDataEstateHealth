...
public class CdeBackfillCatalogRepository : BackfillCatalogRepository, IBackfillCatalogRepository
{
    private readonly string _containerName = "cde";
    private readonly Container _cosmosContainer;
    private readonly IDataEstateHealthRequestLogger _logger;
    private readonly ConcurrentQueue<DataChangeEvent<CriticalDataElement>> _eventQueue;
    ...
    public CdeBackfillCatalogRepository(
        ...
        int batchSize = _cosmosBatchSize) : base(cosmosDatabase)
    {
        ...
        this._eventQueue = new ConcurrentQueue<DataChangeEvent<CriticalDataElement>>();
    ...
    public async Task AddBatch<T>(DataChangeEvent<T> value) where T : class
    {
        ...
        // We're casting here because we know T is CriticalDataElement
    ...
    private async Task ProcessBatchAsync(bool flushAll = false)
    {
        ...
        try
        {
            ...
            var batchItems = new List<DataChangeEvent<CriticalDataElement>>();
    ...
    private async Task ProcessBatchGroupAsync(string accountId, List<DataChangeEvent<CriticalDataElement>> items)
    {
        try
        {
