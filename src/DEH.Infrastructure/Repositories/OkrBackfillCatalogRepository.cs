...
public class OkrBackfillCatalogRepository : BackfillCatalogRepository, IBackfillCatalogRepository
{
    private readonly string _containerName = "okr";
    private readonly Container _cosmosContainer;
    private readonly IDataEstateHealthRequestLogger _logger;
    private readonly ConcurrentQueue<DataChangeEvent<ObjectiveWithAdditionalProperties>> _eventQueue;
    ...
    public OkrBackfillCatalogRepository(
        ...
        int batchSize = _cosmosBatchSize) : base(cosmosDatabase)
    {
        ...
        this._eventQueue = new ConcurrentQueue<DataChangeEvent<ObjectiveWithAdditionalProperties>>();
    ...
    public async Task AddBatch<T>(DataChangeEvent<T> value) where T : class
    {
        ...
        // We're casting here because we know T is ObjectiveWithAdditionalProperties
    ...
    /// <summary>
    ///     Flushes any remaining events in the queue to Cosmos DB
    /// </summary>
    ...
    private async Task ProcessBatchAsync(bool flushAll = false)
    {
        ...
        try
        {
            ...
            var batchItems = new List<DataChangeEvent<ObjectiveWithAdditionalProperties>>();
    ...
    private async Task ProcessBatchGroupAsync(string accountId, List<DataChangeEvent<ObjectiveWithAdditionalProperties>> items)
    {
        try
        {
