...
class CDEColumnAssignment(spark: SparkSession, logger: Logger) {
  ...
  def processCDEColumnAssignment(adlsTargetDirectory: String, schema: org.apache.spark.sql.types.StructType): DataFrame = {
    try {
      ...
      val dfProcess1 = dfRelationship.select(col("SourceType")
        ...
        , col("OperationType")
      ).filter("SourceType='CriticalDataElement' and TargetType='CriticalDataColumn'")
      ...
      case e: Exception =>
        println(s"Error Processing CDECriticalDataColumnAssignment Data: ${e.getMessage}")
        logger.error(s"Error Processing CDECriticalDataColumnAssignment Data: ${e.getMessage}")
        throw e
    }
  }
}
