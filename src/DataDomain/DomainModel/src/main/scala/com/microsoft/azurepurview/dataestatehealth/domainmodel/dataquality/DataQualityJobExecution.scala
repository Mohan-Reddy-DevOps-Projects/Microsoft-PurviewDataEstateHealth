...
class DataQualityJobExecution (spark: SparkSession, logger:Logger){
  ...
  def writeData(df:DataFrame,adlsTargetDirectory:String,refreshType:String,ReProcessingThresholdInMins:Int): Unit = {
    try {
      val EntityName = "DataQualityJobExecution"
          ...
          if (DeltaTable.isDeltaTable(adlsTargetDirectory.concat("/").concat(EntityName))) {
            ...
            if (!df.isEmpty && !dfTarget.isEmpty) {
              ...
              dfTargetDeltaTable.as("target")
                .merge(
                  dfWrite.as("source"),
                  """target.JobExecutionId = source.JobExecutionId""")
                .whenNotMatched()
                .insertAll()
                .execute()
            }
            else if(!df.isEmpty && dfTarget.isEmpty){
              println("Delta Table DataQualityJobExecution Is Empty At Lake For incremental merge. Performing Full Overwrite...")
              dfWrite.write
                .format("delta")
                .mode("overwrite")
                .save(adlsTargetDirectory.concat("/").concat(EntityName))
            ...
            println("Delta Table DataQualityJobExecution Does not exist for incremental merge. Performing Full Overwrite...")
            dfWrite.write
              .format("delta")
              .mode("overwrite")
              .save(adlsTargetDirectory.concat("/").concat(EntityName))
      ...
      case e: Exception =>
        println(s"Error Writing/Merging DataQualityJobExecution data: ${e.getMessage}")
        logger.error(s"Error Writing/Merging DataQualityJobExecution data: ${e.getMessage}")
        throw e
    }
  }
}