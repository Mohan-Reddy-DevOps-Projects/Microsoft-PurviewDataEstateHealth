...
class GlossaryTerm (spark: SparkSession, logger:Logger){
  ...
  def writeData(df:DataFrame,adlsTargetDirectory:String,refreshType:String,ReProcessingThresholdInMins:Int): Unit = {
    try {
      ...
      if (refreshType=="full"){
        ...
        if (DeltaTable.isDeltaTable(adlsTargetDirectory.concat("/").concat(EntityName))) {
          ...
          if (!df.isEmpty && !dfTarget.isEmpty) {
            ...
            println("Delta Table GlossaryTerm Is Empty At Lake For incremental merge. Performing Full Overwrite...")
            dfWrite.write
              .format("delta")
              .mode("overwrite")
              .save(adlsTargetDirectory.concat("/").concat(EntityName))
          ...
          println("Delta Table GlossaryTerm Does not exist for incremental merge. Performing Full Overwrite...")
          dfWrite.write
            .format("delta")
            .mode("overwrite")
            .save(adlsTargetDirectory.concat("/").concat(EntityName))
      ...
      case e: Exception =>
        println(s"Error Writing/Merging GlossaryTerm data: ${e.getMessage}")
        logger.error(s"Error Writing/Merging GlossaryTerm data: ${e.getMessage}")
        throw e
    }
  }
}
