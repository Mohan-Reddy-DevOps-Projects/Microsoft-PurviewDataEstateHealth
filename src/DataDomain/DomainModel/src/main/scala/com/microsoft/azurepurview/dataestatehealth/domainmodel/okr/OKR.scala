  ...
  def processOKR(df:DataFrame,schema: org.apache.spark.sql.types.StructType):DataFrame={
    try{
      val dfProcessUpsert = df.select(col("accountId").alias("AccountId")
        ...
        ,col("payload.after.additionalProperties.OverallStatus").alias("OverallStatus")
        ,col("payload.after.additionalProperties.OverallProgress").alias("OverallProgress")
        ,col("payload.after.additionalProperties.OverallGoal").alias("OverallGoal")
        ,col("payload.after.additionalProperties.OverallMax").alias("OverallMax")
        ,col("payload.after.additionalProperties.KeyResultsCount").alias("KeyResultsCount")
