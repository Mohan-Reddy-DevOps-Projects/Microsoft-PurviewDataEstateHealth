  ...
  def processCDEGlossaryTermAssignment(adlsTargetDirectory: String, schema: org.apache.spark.sql.types.StructType): DataFrame = {
    try {
      ...
      val dfProcess1 = dfRelationship.select(col("SourceType")
        ...
        , col("OperationType")
      ).filter("SourceType='CriticalDataElement' and TargetType='Term'")
      ...
      case e: Exception =>
        println(s"Error Processing CDEGlossaryTermAssignment Data: ${e.getMessage}")
        logger.error(s"Error Processing CDEGlossaryTermAssignment Data: ${e.getMessage}")
        throw e
    }
  }
}
