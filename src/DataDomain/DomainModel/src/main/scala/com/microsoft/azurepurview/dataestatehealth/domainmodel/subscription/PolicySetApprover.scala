...
class PolicySetApprover (spark: SparkSession, logger:Logger){
  ...
  def writeData(df:DataFrame,adlsTargetDirectory:String,refreshType:String,ReProcessingThresholdInMins:Int): Unit = {
    try {
      val EntityName = "PolicySetApprover"
        ...
        if (refreshType=="full"){
          ...
          if (DeltaTable.isDeltaTable(adlsTargetDirectory.concat("/").concat(EntityName))) {
            ...
            if (!df.isEmpty && !dfTarget.isEmpty) {
              ...
              dfTargetDeltaTable.as("target")
                .merge(
                  ...
                  """target.SubscriberRequestId = source.SubscriberRequestId
                    | AND target.AccessPolicySetId = source.AccessPolicySetId
              ...
              println("Delta Table PolicySetApprover Is Empty At Lake For incremental merge. Performing Full Overwrite...")
              dfWrite = dfWrite.filter(lower(col("OperationType")) =!= "delete")
              dfWrite.write
                .format("delta")
                .mode("overwrite")
            ...
            println("Delta Table PolicySetApprover Does not exist for incremental merge. Performing Full Overwrite...")
            dfWrite = dfWrite.filter(lower(col("OperationType")) =!= "delete")
            dfWrite.write
              .format("delta")
              .mode("overwrite")
      ...
      case e: Exception =>
        println(s"Error Writing/Merging PolicySetApprover data: ${e.getMessage}")
        logger.error(s"Error Writing/Merging PolicySetApprover data: ${e.getMessage}")
        throw e
    }
  }
}
