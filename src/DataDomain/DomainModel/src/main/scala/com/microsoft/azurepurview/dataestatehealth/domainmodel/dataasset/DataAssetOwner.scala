...
class DataAssetOwner (spark: SparkSession, logger:Logger){
  def processDataAssetOwner(df:DataFrame,schema: org.apache.spark.sql.types.StructType):DataFrame={
    try{
      ...
      dfProcess = dfProcess
        ...
        .select(col("afterExplodeOwner.id").alias("DataAssetOwner"))
        .filter("DataAssetOwner IS NOT NULL")
        .distinct()

      dfProcess = dfProcess.filter("DataAssetOwner IS NOT NULL").distinct()

      val generateIdColumn = new GenerateId()
      dfProcess = generateIdColumn.IdGenerator(dfProcess,List("DataAssetOwner"),"DataAssetOwnerId")

      dfProcess = dfProcess.select(col("DataAssetOwnerId").cast(StringType)
        ,col("DataAssetOwner").cast(StringType))
      ...
      case e: Exception =>
        println(s"Error Processing DataAssetOwner Data: ${e.getMessage}")
        logger.error(s"Error Processing DataAssetOwner Data: ${e.getMessage}")
        throw e
    }
  }
}
