...
class DataProductOwnershipTrustedData (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlDataProductOwnershipTrustedData(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataProductOwnershipTrustedData",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataProductOwnershipTrustedData",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
        ...
        var dfProcess = spark.sql(
          """SELECT (SELECT HealthControlId FROM DimDataHealthControl
            | WHERE HealthControlDisplayName='Data Product Ownership'
        ...
        dfProcess = dfProcess.select(
          ...
          col("DataProductOwnerScore").cast(DoubleType),
          col("DataProductHasOwner").cast(IntegerType),
          col("DataProductDataAssetHasOwnerCount").cast(IntegerType),
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlDataProductOwnershipTrustedData Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlDataProductOwnershipTrustedData Control: ${e.getMessage}")
        throw e
    }
  }
}
