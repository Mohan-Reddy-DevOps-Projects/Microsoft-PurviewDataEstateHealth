...
class DataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlDataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
        ...
        var dfProcess = spark.sql(
          """SELECT (SELECT HealthControlId FROM DimDataHealthControl
            | WHERE HealthControlDisplayName='Data estate health monitoring, alerting, and insights'
            | AND HealthControlGroupDisplayName='Estate Observability') AS HealthControlId
        ...
        dfProcess = dfProcess.select(
          ...
          lit(1.00000).cast(DoubleType).as("DataProductObservabilityScore"),
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlDataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlDataEstateHealthMonitoringAlertingAndInsightsDataEstateHealthObservability Control: ${e.getMessage}")
        throw e
    }
  }
}
