...
class DataProductCertification (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlDataProductCertification(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      val listOfDimensionalAssets = List(
        ...
        , "DimBusinessDomain"
        , "DimDate")
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataProductCertification",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataProductCertification",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
      ...
      var dfProcess = spark.sql("""SELECT (SELECT HealthControlId FROM DimDataHealthControl
                                  | WHERE HealthControlDisplayName='Data Product Certification'
                                  ...
                                  |,ControlQuery.DataProductCertifiedCount AS DataProductCertifiedCount
                                  |FROM (
                                  ...
                                  |,CAST(CASE WHEN DP.Endorsed IS TRUE THEN 1 ELSE 0 END AS INT) AS DataProductCertifiedCount
                                  |FROM DataProduct DP
      ...
      dfProcess = dfProcess.select(
        ...
        col("DataProductCertifiedCount").cast(IntegerType),
      ...
      case e: Exception =>
        println(s"Error Processing processFactDataGovernanceScanControlDataProductCertification Control: ${e.getMessage}")
        logger.error(s"Error Processing processFactDataGovernanceScanControlDataProductCertification Control: ${e.getMessage}")
        throw e
    }
  }
}
