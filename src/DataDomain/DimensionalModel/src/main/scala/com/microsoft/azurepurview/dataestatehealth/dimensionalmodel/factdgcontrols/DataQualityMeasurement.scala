...
class DataQualityMeasurement (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlDataQualityMeasurement(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataQualityMeasurement",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DataQualityMeasurement",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
      ...
      var dfProcess = spark.sql("""SELECT (SELECT HealthControlId FROM DimDataHealthControl
                                  | WHERE HealthControlDisplayName='Data Quality Measurement'
                                  | AND HealthControlGroupDisplayName='Trusted Data') AS HealthControlId
                                  ...
                                  |,ControlQuery.DataProductAssetDQEnabledCount AS DataProductAssetDQEnabledCount
                                  |,ControlQuery.DataProductAssetDQNotEnabledCount AS DataProductAssetDQNotEnabledCount
                                  |,ControlQuery.DataProductAssetDQEnabledScore AS DataProductAssetDQEnabledScore
                                  | FROM (
                                  | SELECT DP.DataProductId
                                  |,BD.BusinessDomainId
                                  |,COALESCE(DQDetails.DataProductAssetDQEnabledCount,0) AS DataProductAssetDQEnabledCount
                                  |,COALESCE(AC.DataProductAssetCount,0) - COALESCE(DQDetails.DataProductAssetDQEnabledCount,0) AS  DataProductAssetDQNotEnabledCount
                                  |,COALESCE(AC.DataProductAssetCount,0) AS TotalDataProductAssetCount
                                  |,1 AS DataProductCounter
                                  |, CASE WHEN COALESCE(AC.DataProductAssetCount,0) = 0 THEN 0
                                  |       ELSE CAST(COALESCE(DQDetails.DataProductAssetDQEnabledCount,0) AS DECIMAL)/ COALESCE(AC.DataProductAssetCount,0)
                                  |	   END AS DataProductAssetDQEnabledScore
                                  ...
                                  | LEFT JOIN
                                  |(SELECT DPAA.DataProductId
                                  | ,COALESCE(COUNT(DAF.DataAssetId),0) AS DataProductAssetDQEnabledCount
                                  | FROM DataProductAssetAssignment DPAA
                                  | LEFT JOIN (
                                  | SELECT DISTINCT DA.DataAssetId  FROM
                                  |									(SELECT Distinct DataAssetId FROM DataQualityRuleColumnExecution
                                  |									UNION
                                  |									SELECT Distinct DataAssetId FROM DataQualityAssetRuleExecution)DQA
                                  | INNER JOIN DataAsset DA
                                  | ON DQA.DataAssetId = DA.DataAssetId
                                  |) DAF
                                  | ON DPAA.DataAssetId = DAF.DataAssetId
                                  | GROUP BY DPAA.DataProductId) DQDetails
                                  | ON DP.DataProductId = DQDetails.DataProductId
                                  | LEFT JOIN (SELECT DPAA.DataProductId, COUNT(DA.DataAssetId) DataProductAssetCount
                                  |			FROM DataAsset DA
      ...
      dfProcess = dfProcess.select(
        ...
        col("DataProductAssetDQEnabledCount").cast(IntegerType),
        col("DataProductAssetDQNotEnabledCount").cast(IntegerType),
        col("DataProductAssetDQEnabledScore").cast(DoubleType),
    ...
    else
    {
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlClassificationAndLabeling Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlClassificationAndLabeling Control: ${e.getMessage}")
        throw e
    }
  }
