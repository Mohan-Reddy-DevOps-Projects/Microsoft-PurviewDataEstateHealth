...
class LinkedAssetsMetadataQualityManagement (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlLinkedAssetsMetadataQualityManagement(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-LinkedAssetsMetadataQualityManagement",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-LinkedAssetsMetadataQualityManagement",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
        ...
        var dfProcess = spark.sql(
          """SELECT (SELECT HealthControlId FROM DimDataHealthControl
            | WHERE HealthControlDisplayName='Linked Assets'
            ...
            |,ControlQuery.DataProductHasAtLeast1Asset
            |,ControlQuery.DataProductHasAssoPubTerm
            |,ControlQuery.LinkedAssetsScore
            | FROM (
            ...
            |, CASE WHEN BD.BusinessDomainId IS NULL THEN 0 ELSE 1 END DataProductHasParentBusinessDomain
            |, CASE WHEN COALESCE(AC.DataProductAssetCount,0) > 0 THEN 1 ELSE 0 END DataProductHasAtLeast1Asset
            |, CASE WHEN GTDP.PublishedGlossaryTermExists IS NULL THEN 0 ELSE 1 END  AS DataProductHasAssoPubTerm
            |, CAST((CASE WHEN BD.BusinessDomainId IS NULL THEN 0 ELSE 1 END
            |  + CASE WHEN COALESCE(AC.DataProductAssetCount,0) > 0 THEN 1 ELSE 0 END
            |  + CASE WHEN GTDP.PublishedGlossaryTermExists IS NULL THEN 0 ELSE 1 END) AS DECIMAL)/3 AS LinkedAssetsScore
            | FROM DataProduct DP
        ...
        dfProcess = dfProcess.select(
          ...
          col("DataProductHasAtLeast1Asset").cast(IntegerType),
          col("DataProductHasAssoPubTerm").cast(IntegerType),
          col("LinkedAssetsScore").cast(DoubleType),
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlLinkedAssetsMetadataQualityManagement Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlLinkedAssetsMetadataQualityManagement Control: ${e.getMessage}")
        throw e
    }
  }
}
