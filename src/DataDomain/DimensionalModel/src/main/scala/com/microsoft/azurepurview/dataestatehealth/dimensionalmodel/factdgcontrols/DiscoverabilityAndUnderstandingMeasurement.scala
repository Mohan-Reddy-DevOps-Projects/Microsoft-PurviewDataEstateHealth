...
class DiscoverabilityAndUnderstandingMeasurement (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlDiscoverabilityAndUnderstandingMeasurement(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      val listOfDimensionalAssets = List(
        ...
        , "DimBusinessDomain"
        , "DimDate")
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DiscoverabilityAndUnderstandingMeasurement",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-DiscoverabilityAndUnderstandingMeasurement",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
      ...
      var dfProcess = spark.sql("""SELECT (SELECT HealthControlId FROM DimDataHealthControl
                                  | WHERE HealthControlDisplayName='Discoverability and Understanding Measurement'
                                  | AND HealthControlGroupDisplayName='Data Discoverability and Understanding (MDQ)') AS HealthControlId
                                  ...
                                  |,ControlQuery.DataProductAssetMDQEnabledCount AS DataProductAssetMDQEnabledCount
                                  |,ControlQuery.DataProductAssetMDQNotEnabledCount AS DataProductAssetMDQNotEnabledCount
                                  |,ControlQuery.DataProductAssetMDQEnabledScore AS DataProductAssetMDQEnabledScore
                                  ...
                                  |,ControlQuery.DataProductHasGlossaryTerm AS DataProductHasGlossaryTerm
                                  ...
                                  |,1 AS DataProductAssetMDQEnabledCount
                                  |,0 AS DataProductAssetMDQNotEnabledCount
                                  |,CAST(1 AS DECIMAL)/1 AS DataProductAssetMDQEnabledScore
                                  ...
                                  |, CASE WHEN GT.DataProductId IS NOT NULL THEN 1 ELSE 0 END AS DataProductHasGlossaryTerm
                                  ...
                                  |LEFT JOIN
                                  |(SELECT DISTINCT DP.DataProductId
                                  | FROM GlossaryTermDataProductAssignment GTDPA
                                  | INNER JOIN DataProduct DP
                                  | ON DP.DataProductId = GTDPA.DataProductId) GT
      ...
      dfProcess = dfProcess.select(
        ...
        col("DataProductHasGlossaryTerm").cast(IntegerType),
        ...
        col("DataProductAssetMDQEnabledCount").cast(IntegerType),
        col("DataProductAssetMDQNotEnabledCount").cast(IntegerType),
        col("DataProductAssetMDQEnabledScore").cast(DoubleType),
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlDiscoverabilityAndUnderstandingMeasurement Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlDiscoverabilityAndUnderstandingMeasurement Control: ${e.getMessage}")
        throw e
    }
  }
