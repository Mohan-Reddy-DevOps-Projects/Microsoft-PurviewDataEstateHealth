...
class CompliantDataUse (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlCompliantDataUse(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      val listOfDimensionalAssets = List(
        ...
        , "DimBusinessDomain"
        , "DimDate")
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-CompliantDataUse",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-CompliantDataUse",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
      ...
      var dfProcess = spark.sql("""SELECT (SELECT HealthControlId FROM DimDataHealthControl
                                  | WHERE HealthControlDisplayName='Compliant Data Use'
                                  ...
                                  |,ControlQuery.DataProductComplianceDataTermsOfUsePolicyScore AS DataProductComplianceDataTermsOfUsePolicyScore
                                  |,ControlQuery.DataProductComplianceDataTermsOfUsePolicyCount AS DataProductComplianceDataTermsOfUsePolicyCount
                                  |
                                  |FROM (
                                  ...
                                  |,CAST(COALESCE(DPTOU.TermsOfUseCount,0) AS DECIMAL)/1 AS DataProductComplianceDataTermsOfUsePolicyScore
                                  |,COALESCE(DPTOU.TermsOfUseCount,0)  AS DataProductComplianceDataTermsOfUsePolicyCount
                                  |FROM DataProduct DP
                                  ...
                                  |LEFT JOIN
                                  |(SELECT DISTINCT DataProductId, 1 TermsOfUseCount
                                  |FROM DataProductTermsOfUse) DPTOU
                                  |ON DP.DataProductId = DPTOU.DataProductId
      ...
      dfProcess = dfProcess.select(
        ...
        col("DataProductComplianceDataTermsOfUsePolicyScore").cast(DoubleType),
        col("DataProductComplianceDataTermsOfUsePolicyCount").cast(IntegerType),
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlCompliantDataUse Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlCompliantDataUse Control: ${e.getMessage}")
        throw e
    }
  }
}
