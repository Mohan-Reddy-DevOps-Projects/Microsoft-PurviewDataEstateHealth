...
class ClassificationAndLabeling (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlClassificationAndLabeling(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-ClassificationAndLabeling",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-ClassificationAndLabeling",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
        ...
        var dfProcess = spark.sql(
          """SELECT (SELECT HealthControlId FROM DimDataHealthControl
            | WHERE HealthControlDisplayName='Classification and Labeling'
            | AND HealthControlGroupDisplayName='Estate Curation') AS HealthControlId
        ...
        dfProcess = dfProcess.select(
          ...
          col("ClassifiedAssetCount").cast(IntegerType),
          col("DataProductClassificationScore").cast(DoubleType),
          col("DataProductIsClassified").cast(IntegerType),
