...
class SelfServeAccessEnablement (spark: SparkSession, logger:Logger){
  def processFactDataGovernanceScanControlSelfServeAccessEnablement(schema: org.apache.spark.sql.types.StructType,targetAdlsRootPath:String):DataFrame=  {
    try {
      ...
      if (factDependencySoftCheck.checkAssets("FactDataGovernanceScan-SelfServeAccessEnablement",targetAdlsRootPath.concat("/").concat("DomainModel"),listOfDomainAssetsFactDepCheck)
        && factDependencySoftCheck.checkAssets("FactDataGovernanceScan-SelfServeAccessEnablement",targetAdlsRootPath.concat("/").concat("DimensionalModel"),listOfDimensionalAssets))
      {
      ...
      var dfProcess = spark.sql("""SELECT (SELECT HealthControlId FROM DimDataHealthControl
                                  | WHERE HealthControlDisplayName='Self-serve Access Enablement'
                                  | AND HealthControlGroupDisplayName='Access And Use') AS HealthControlId
                                  ...
                                  |,ControlQuery.DataProductSelfServiceAccessPolicyCount AS DataProductSelfServiceAccessPolicyCount
                                  |,ControlQuery.DataProductSelfServiceEnablementScore AS DataProductSelfServiceEnablementScore
                                  | FROM (
                                  ...
                                  |,COALESCE(DAPS.DataProductSelfServiceAccessPolicyCount,0) AS DataProductSelfServiceAccessPolicyCount
                                  |,CAST(COALESCE(DAPS.DataProductSelfServiceAccessPolicyCount,0) AS DECIMAL)/1 AS DataProductSelfServiceEnablementScore
                                  |FROM DataProduct DP
                                  ...
                                  |LEFT JOIN
                                  |(SELECT PolicyAppliedOnId AS DataProductId, 1 AS DataProductSelfServiceAccessPolicyCount
                                  |FROM AccessPolicySet
                                  |WHERE PolicyAppliedOn ='DataProduct') DAPS
                                  |ON DP.DataProductId = DAPS.DataProductId
      ...
      dfProcess = dfProcess.select(
        ...
        col("DataProductSelfServiceEnablementScore").cast(DoubleType),
        col("DataProductSelfServiceAccessPolicyCount").cast(IntegerType),
    ...
    else
    {
      ...
      case e: Exception =>
        println(s"Error Processing FactDataGovernanceScanControlSelfServeAccessEnablement Control: ${e.getMessage}")
        logger.error(s"Error Processing FactDataGovernanceScanControlSelfServeAccessEnablement Control: ${e.getMessage}")
        throw e
    }
  }
}
