...
class CdmcControlsAggregation:
    def aggregate_cdmc_controls_by_domain(dataproduct_df,productdomain_association_df):
        ...
        dataproduct_df = dataproduct_df.select("BusinessDomainId","C2_Ownership","C3_AuthoritativeSource","C5_Catalog",
        ...
        cdmc_controls_by_domain_df = final_aggregated_cdmc_control_domain_df.select("BusinessDomainId","Ownership","AuthoritativeSource","Catalog",
                                "Classification","Access","DataConsumptionPurpose","Quality")
        
        return cdmc_controls_by_domain_df
    
    def aggregate_cdmc_controls(cdmc_controls_df_by_domain):
        
        #calculate aggregated cdmc control for all domains
        cdmc_controls_df = CdmcControlsTransformations.calculate_aggregated_cdmc_column_sum(cdmc_controls_df_by_domain)
        
        cdmc_controls_df = cdmc_controls_df.select("Ownership","AuthoritativeSource","Catalog",
                                "Classification","Access","DataConsumptionPurpose","Quality")
