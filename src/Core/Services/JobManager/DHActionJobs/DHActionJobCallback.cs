...
[JobCallback(Name = nameof(DHActionJobCallback))]
internal class DHActionJobCallback : StagedWorkerJobCallback<ActionCleanUpJobMetadata>
{
    private readonly IDataEstateHealthRequestLogger dataEstateHealthRequestLogger;

    /// <inheritdoc />
    protected override string JobName => nameof(DHActionJobCallback);

    public DHActionJobCallback(IServiceScope scope)
        : base(scope)
    {
    ...
    protected override void OnJobConfigure()
    {
        ...
        {
            new DHActionJobStage(this.Scope, this.Metadata, this.JobCallbackUtils)
        };
    }
    ...
    {
        this.Metadata.ActionCleanUpCompleted = false;
    }
}
