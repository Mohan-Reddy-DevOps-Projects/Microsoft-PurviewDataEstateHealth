...
[JobCallback(Name = nameof(MDQFailedJobCallback))]
internal class MDQFailedJobCallback : StagedWorkerJobCallback<MDQFailedJobMetadata>
{
    private readonly IDataEstateHealthRequestLogger dataEstateHealthRequestLogger;

    /// <inheritdoc />
    protected override string JobName => nameof(MDQFailedJobCallback);

    public MDQFailedJobCallback(IServiceScope scope)
        : base(scope)
    {
    ...
    protected override void OnJobConfigure()
    {
        ...
        {
            new MDQFailedJobStage(this.Scope, this.Metadata, this.JobCallbackUtils)
        };
    }
    ...
    {
        this.Metadata.MDQFailedJobProcessed = false;
    }
}
