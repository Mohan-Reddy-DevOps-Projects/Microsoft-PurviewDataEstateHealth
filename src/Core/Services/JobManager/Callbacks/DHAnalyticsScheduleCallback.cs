...
[JobCallback(Name = nameof(DHAnalyticsScheduleCallback))]
internal class DHAnalyticsScheduleCallback : StagedWorkerJobCallback<DEHScheduleJobMetadata>
{
    private readonly IDataEstateHealthRequestLogger dataEstateHealthRequestLogger;

    /// <inheritdoc />
    protected override string JobName => nameof(DHAnalyticsScheduleCallback);

    public DHAnalyticsScheduleCallback(IServiceScope scope)
        : base(scope)
    {
    ...
    protected override void OnJobConfigure()
    {
        ...
        this.dataEstateHealthRequestLogger.LogInformation($"{this.JobName} Job Configure Executed.");
    }
