...
internal class TrackAnalyticsSparkJobStage : IJobCallbackStage
{
    ...
    public TrackAnalyticsSparkJobStage(
    IServiceScope scope,
    ...
    public string StageName => nameof(TrackAnalyticsSparkJobStage);
    ...
    public async Task<JobExecutionResult> Execute()
    {
        ...
        try
        {
            ...
            this.logger.LogTrace($"track dimension analytics spark job stage status: {jobStatusMessage}");
        ...
        {
            jobStageStatus = JobExecutionStatus.Completed;
            jobStatusMessage = $"{this.StageName}|Failed to track Analytics SPARK job for account: {this.metadata.AccountServiceModel.Id} in {this.StageName} with error: {exception.Message}";
