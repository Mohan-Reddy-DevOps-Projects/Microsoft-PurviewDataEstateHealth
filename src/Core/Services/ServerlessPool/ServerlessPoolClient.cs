...
{
    private readonly AadAppTokenProviderService<ServerlessPoolAuthConfiguration> aadService;
    ...
    public ServerlessPoolClient(
        AadAppTokenProviderService<ServerlessPoolAuthConfiguration> aadService,
    ...
    public async Task<int> ExecuteCommandAsync(string query, CancellationToken cancellationToken, int connectTimeout = 15, int commandTimeout = 15)
    {
        ...
        AuthenticationResult accessToken = await this.aadService.GetTokenAsync(this.authConfig.Resource);

        return await this.ExecuteNonQueryAsync(query, connectionStringBuilder, accessToken.AccessToken, commandTimeout, cancellationToken);
    }
    ...
    public async IAsyncEnumerable<IDataRecord> ExecuteQueryAsync(string query, string database, [EnumeratorCancellation] CancellationToken cancellationToken, int connectTimeout = 15, int commandTimeout = 15)
    {
        ...
        IAsyncEnumerable<IDataRecord> records = this.AccessDatabaseAsync(query, connectionStringBuilder, accessToken.AccessToken, commandTimeout, cancellationToken);
